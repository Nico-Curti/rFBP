...
void Cavity_Message < Mag > :: save_weights(const std :: string & filename)
{
  ...
  for (long int i = 0L; i < this->K; ++i)
    for (long int j = 0L; j < this->N; ++j)
      os.write( reinterpret_cast <char *>(&this->m_j_star[i][j].value), sizeof( double ));
  os.close();
}
...
{ // WHAT ABOUT FMT???
  ...
  for (long int i = 0L; i < this->K; ++i)
    for (long int j = 0L; j < this->N; ++j)
      os.write( reinterpret_cast<const char *> (&this->m_j_star[i][j].value), sizeof( double ));

  for (long int i = 0L; i < this->K; ++i)
    for (long int j = 0L; j < this->N; ++j)
      os.write( reinterpret_cast<const char *> (&this->m_star_j[i][j].value), sizeof( double ));
  ...
  for (long int i = 0L; i < this->M; ++i)
    for (long int j = 0L; j < this->K; ++j)
      os.write( reinterpret_cast<const char *> (&this->m_in[i][j].value), sizeof( double ));

  for (long int i = 0L; i < this->M; ++i)
    for (long int j = 0L; j < this->K; ++j)
      for (long int k = 0L; k < this->N; ++k)
        os.write( reinterpret_cast<const char *> (&this->weights[i][j][k].value), sizeof( double ));

  for (long int i = 0L; i < this->M; ++i)
    for (long int j = 0L; j < this->K; ++j)
      os.write( reinterpret_cast<const char *> (&this->m_no[i][j].value), sizeof( double ));

  for (long int i = 0L; i < this->M; ++i)
    os.write( reinterpret_cast<const char *> (&this->m_on[i].value), sizeof( double ));

  for (long int i = 0L; i < this->M; ++i)
    for (long int j = 0L; j < this->K; ++j)
      os.write( reinterpret_cast<const char *> (&this->m_ni[i][j].value), sizeof( double ));
